# Qubic France – Community CCF Proposal: **Crypto XR 2026**

## Sprint Details
- **Event Date**: **15th January 2026 –  17th January 2026**  
- **Funding Request**: **14,3 billion QUBIC** @ $1 500 / billion ($22 500 USDT)(all extra cost will be refund CCF)
- **Wallet**: `PQXFNXBUHNSLEDQNALBCSTQKJNHBPWOQLGSDXQPIMAQQJAZSWEHLOWKAYIFB`

### Available Options
| Option | Description |
| ------ | ----------- |
| **0** | No, I don’t want |
| **1** | Yes, approve the budget of **14,3 billion QUBIC** |

**Use of Funds**: Paid media exposure only - no internal salaries or operations  - funds returned to the CCF in case of cancellation or excess funds  
**Project Team**: **Qubic France**, *Sebit* (project lead)

(20% discount applied on the entire order if payment is made before October 15, 2025)
---

### TL;DR
Requesting **14,3 billion QUBIC** (~ $22,5 k; any price difference will be returned to CCF) for a high-impact.

# 🚀 Why Qubic Should Attend

- **Unmatched Visibility in the French Market**  
  CryptoXR gathered over 6,800 attendees in 2025. Qubic’s presence would strengthen its branding in one of the most crypto-active regions in Europe.

- **Direct Engagement with Target Audiences**  
  The event splits between B2B (Day 1) and B2C (Day 2). We can connect with investors, exchanges, developers, and community members. Crypto XR is the most anticipated event of the year in France and is seen as the place to be for networking, with all French KOLs and most European media in attendance.

- **Workshops & Speaker Opportunities**  
  Qubic can propose a live demo or talk around its unique architecture (*Aigarth, Computors, Useful PoW*). A 15-minute slot as a speaker is also reserved on the main stage during a time slot with a good audience.

- **Qubic France Community Activation**  
  We can mobilize our local contributors, attract new ambassadors, and distribute educational and promotional materials.

We have been working on this event since early 2025, notably through partnerships with three media outlets, certain KOLs (Cryptolyze, Befresh, CryptoPourTous, Les Rois du Bitcoin, etc.), as well as a partnership negotiated with Epitech, one of the largest developer schools in France, which we were able to contact thanks to Rayan (Qulang).
We therefore hope to consolidate our year with this event so that we can continue to pursue our goals in 2026.

---

# 🔍 What’s in It for Qubic?

1. **Brand Elevation & Market Leadership**  
   Participating at France’s foremost Web3 & AI event positions Qubic as an accessible, forward-thinking player in the national and European crypto ecosystem.  
   It amplifies our visibility among key stakeholders from institutional partners to grassroots enthusiasts.

2. **Targeted Networking & Business Development**  
   CryptoXR’s dual B2B and B2C format enables us to engage with investors, exchanges, infrastructure providers, and crypto-curious individuals—maximizing impact across our audience spectrum.

3. **Thought Leadership & Product Showcase**  
   By delivering a talk or workshop, Qubic can highlight innovations like *Aigarth* or *Useful PoW*.  
   A live demo could create real-time engagement.

4. **Community Activation & Ambassador Recruitment**  
   We can energize the French-speaking community, reward contributors, and onboard new members interactively.

5. **Media Exposure & Brand Recall**  
   CryptoXR offers built-in marketing channels to extend Qubic’s reach through event videos, interviews, and promotional content.

6. **ROI Through Engagement & Conversion**  
   Affordable packages and immersive formats ensure impactful exposure and strong return on investment already made thanks to a low-cost booth that can be reused for future events in France, We will co-organize a side event with a school, a mining company, and a French marketing agency. 
---

> **“Participating in CryptoXR 2026 offers Qubic a low-cost, high-impact opportunity to elevate our brand in the French and European crypto ecosystem. Through strategically placed engagement—booth presence, live demo, workshop, and community activation—we can generate high-value leads, deepen community bonds, attract ambassadors, and secure lasting visibility via media and event coverage.”**

# 📦 Proposed Setup

| Element                     | Description                                                                 |
|-----------------------------|-----------------------------------------------------------------------------|
| 🎪 **Booth / Stand**        | A branded Qubic space to onboard, demo, and discuss. The stand will reusable for differents events in Europe with a short cost than the first one.                        |
| 🎤 **Speaker Slot / Workshop** | Presentation of the Qubic ecosystem, roadmap, or Aigarth integration        |
| 👥 **Delegation**           | 5 key members of Qubic France to represent and coordinate onsite efforts + 1 Qubic Staff |
| 📸 **Media Coverage**       | Event video recap, interviews, and post-event content (like RaiseHack)                       |
| 🧩 **Merchandising**        | Stickers, flyers, shirts, or QR codes to join Discord / try demos            |


### Cost Breakdown
| Item | Cost (USD) |
|------|------------|
| Co-host side event (800+ people expected)  | $5180.00 |
| Reusable by EMEA Stand | $8000.00 |
| Team on site Expenses coverage (hotel, food, travel...) | $2960.00 |
| Participation on workshop (45mn) + speaker (15mn) with Qubic team | $4000.00 |
| Goodies on site | $2000.00 |
|200 million QUBIC Giveaway | SPOT |
| **Total** | **22500.00** |

 

### Reusable Stand

Our vision is to have a Qubic stand reusable for all EMEA communities. The stand will be store by a partner freely. It could be ship and setup everywhere in EMEA.

Please consider this stand as Community Wallet.

![stand1](https://github.com/QubicFrance/Medias/blob/9361a7a92db55664726bc42ab2a79a2658ccf6f7/Stand%20_Event_Qubic_Vue_1.jpg)
![stand2](https://github.com/QubicFrance/Medias/blob/9361a7a92db55664726bc42ab2a79a2658ccf6f7/Stand_Event_Vue_2.jpg)

# KPI CryptoXR 2026 - Qubic France

## Visibility & Exposure Metrics

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **Booth Visitors** | 1,500+ visitors | Manual counting + QR codes scanned + goodies |
| **Leads Generated** | 500+ qualified contacts | Forms, card exchanges, registrations |
| **Media Impressions** | 300,000+ views | Analytics from videos, articles, social posts |
| **Press Mentions** | 5+ articles/8+ interviews | Media monitoring |

## Engagement & Interaction

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **Workshop Attendees** | 100+ people |Counting during 45-min workshop |
| **Presentation Audience** | 300+ people | Counting during 15-min speaker slot |
| **Merchandise Distributed** | 500+ items | Before/after inventory |

## Community Growth

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **New Discord Members** | 1,000+ via QR codes | Discord analytics + goodies |
| **New Ambassadors** | 10+ recruited + 200~500 community members | Ambassador database + goodies |
| **Social Media Followers** | +1,000 Qubic +1.000 Qubic France | Platform analytics |
| **Newsletters** | 100k (cointribune + CryptoXR) | Email analytics |

## ROI & Conversion

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **QUBIC Distributed** | 200M QUBIC giveaway | Distribution tracking |
| **Partnerships Established** | 10+ new partners | Commercial follow-up + KOL |
| **Devs Interest** | 3+ | Community SC + Ecosystem RFP |

## Side Event (Co-hosted)

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **Side Event Participants** | 800+ people | Entry counting |
| **Qualified Networking** | 100+ scheduled meetings | Participant agenda |
| **Partners Mobilized** | School + Mining + Agency + Media (Cointribune) | Participation confirmation + article |

## Post-Event Metrics

| KPI | Target | Measurement Method |
|-----|--------|--------------------|
| **Event Videos** | 100,000+ views/video | YouTube/social medias |
| **User-Generated Content** | 50+ community posts | Hashtag monitoring |
| **Business Pipeline** | $100K+ opportunities | tracking/contact |
| **Stand Reusability** | 3+ EMEA events 2026 | Event planning schedule |

## Strategic Objectives

| Objective | Metric | Target |
|-----------|--------|--------|
| **French Market Leadership** | Share of voice vs competitors | Top 3 mentions |
| **Developer Ecosystem** | Engaged Epitech/agency developers | 50+ developers (school) + agency |
| **Technology Adoption** | demos | 200+ people reached |
| **European Presence** | EMEA media coverage | 5+ countries covered |
| **Community Maturity** | Networking | 3 new big and low cost event in 2026 + small events via ambassadors |
| **Country Hype** | KOL + Media | 10+ new partnership (20 videos) + 2 new media in contact for free (2 articles) |


## Budget vs Performance

**Allocated Budget**: 14,3 billion QUBIC ($22,500 USD)

**Expected ROI**:
  - Cost per impression: < $0.05
    
  - Cost per qualified lead: < $4.5
    
  - Return on investment: 300%+ in generated value

**Financial Tracking**: All surplus funds will be returned to CCF


These goals align with QUBIC’s objectives of ecosystem growth, stakeholder engagement, and technology adoption.

*All funds go to external paid exposure. No team salaries or overhead.*


# Qubic France – 社群 CCF 提案：**Crypto XR 2026**

## 衝刺詳情
- **活動日期**：**2026年1月15日 –  2026年1月17日**  
- **資金申請**：**143億 QUBIC** @ $1 500 / 十億 ($22 500 USDT)（所有額外費用將退還給CCF）
- **錢包地址**：`PQXFNXBUHNSLEDQNALBCSTQKJNHBPWOQLGSDXQPIMAQQJAZSWEHLOWKAYIFB`

### 可選選項
| 選項 | 描述 |
| ------ | ----------- |
| **0** | 否，我不希望 |
| **1** | 是，批准**143億 QUBIC**的預算 |

**資金用途**：僅用於付費媒體曝光 - 無內部薪水或營運費用 - 如取消或有剩餘資金將退還給CCF  
**專案團隊**：**Qubic France**，*Sebit*（專案負責人）

（如在2025年10月15日前付款，整個訂單享有20%折扣）
---

### TL;DR
申請**143億 QUBIC**（約 $22,5 k；任何價格差異將退還給CCF）用於高影響力項目。

# 🚀 為什麼Qubic應該參加

- **在法國市場無與倫比的可見度**  
  CryptoXR在2025年聚集了超過6,800名參與者。Qubic的參與將加強其在歐洲最活躍的加密貨幣地區之一的品牌形象。

- **與目標受眾的直接互動**  
  活動分為B2B（第1天）和B2C（第2天）。我們可以與投資者、交易所、開發者和社群成員建立聯繫。Crypto XR是法國年度最受期待的活動，被視為網絡交流的必到之地，所有法國KOL和大多數歐洲媒體都會出席。

- **研討會與演講機會**  
  Qubic可以提出圍繞其獨特架構的現場演示或演講（*Aigarth、Computors、有用的PoW*）。在觀眾較多的時段，主舞台還預留了15分鐘的演講時段。

- **Qubic France社群激活**  
  我們可以動員本地貢獻者，吸引新大使，並分發教育和宣傳材料。

我們從2025年初就開始為這個活動工作，特別是通過與三個媒體機構的合作夥伴關係，某些KOL（Cryptolyze、Befresh、CryptoPourTous、Les Rois du Bitcoin等），以及與Epitech（法國最大的開發者學校之一）談判的合作夥伴關係，我們能夠通過Rayan（Qulang）聯繫到他們。
因此，我們希望通過這個活動來鞏固我們的一年，以便我們能夠在2026年繼續追求我們的目標。

---

# 🔍 這對Qubic有什麼好處？

1. **品牌提升與市場領導地位**  
   參加法國首屈一指的Web3和AI活動，將Qubic定位為國家和歐洲加密生態系統中一個易於接近、具有前瞻性思維的參與者。  
   它增強了我們在從機構合作夥伴到基層愛好者等關鍵利益相關者中的可見度。

2. **定向網絡建設與業務發展**  
   CryptoXR的雙重B2B和B2C格式使我們能夠與投資者、交易所、基礎設施提供商和對加密貨幣好奇的個人互動—最大化我們對受眾群體的影響。

3. **思想領導地位與產品展示**  
   通過發表演講或主持研討會，Qubic可以突出*Aigarth*或*有用的PoW*等創新。  
   現場演示可以創造實時互動。

4. **社群激活與大使招募**  
   我們可以激勵法語社群，獎勵貢獻者，並互動式地吸納新成員。

5. **媒體曝光與品牌回憶**  
   CryptoXR提供內建的營銷渠道，通過活動視頻、採訪和宣傳內容擴展Qubic的影響力。

6. **通過參與與轉化實現ROI**  
   實惠的套餐和沉浸式格式確保有影響力的曝光和強勁的投資回報，這要歸功於可重複用於法國未來活動的低成本展位，我們將與一所學校、一家挖礦公司和一家法國營銷機構共同組織一個側邊活動。
---

> **"參加CryptoXR 2026為Qubic提供了一個低成本、高影響力的機會，在法國和歐洲加密生態系統中提升我們的品牌。通過戰略性的參與—展位存在、現場演示、研討會和社群激活—我們可以產生高價值的潛在客戶，加深社群聯繫，吸引大使，並通過媒體和活動報導獲得持久的可見度。"**

# 📦 建議設置

| 元素                     | 描述                                                                 |
|-----------------------------|-----------------------------------------------------------------------------|
| 🎪 **展位 / 展台**        | 一個品牌化的Qubic空間，用於引導、演示和討論。該展台可重複用於歐洲不同活動，成本比第一次要低。                        |
| 🎤 **演講時段 / 研討會** | 介紹Qubic生態系統、路線圖或Aigarth整合        |
| 👥 **代表團**           | Qubic France的5名核心成員代表並協調現場工作 + 1名Qubic員工 |
| 📸 **媒體報導**       | 活動視頻回顧、採訪和活動後內容（如RaiseHack）                       |
| 🧩 **商品**        | 貼紙、傳單、T恤或加入Discord / 嘗試演示的QR碼            |


### 成本細分
| 項目 | 成本（美元） |
|------|------------|
| 共同主辦側邊活動（預計800+人）  | $5180.00 |
| 可重複使用的EMEA展台 | $8000.00 |
| 現場團隊費用報銷（酒店、餐飲、交通...） | $2960.00 |
| 與Qubic團隊參與研討會（45分鐘）+ 演講（15分鐘） | $4000.00 |
| 現場贈品 | $2000.00 |
|2億QUBIC贈送 | 現貨 |
| **總計** | **22500.00** |

 

### 可重複使用的展台

我們的願景是擁有一個可供所有EMEA社群重複使用的Qubic展台。該展台將由合作夥伴免費存儲。它可以運送並在EMEA任何地方設置。

請將此展台視為社群錢包。

![stand1](https://github.com/QubicFrance/Medias/blob/9361a7a92db55664726bc42ab2a79a2658ccf6f7/Stand%20_Event_Qubic_Vue_1.jpg)
![stand2](https://github.com/QubicFrance/Medias/blob/9361a7a92db55664726bc42ab2a79a2658ccf6f7/Stand_Event_Vue_2.jpg)

# KPI CryptoXR 2026 - Qubic France

## 可見度與曝光度指標

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **展位訪客** | 1,500+ 訪客 | 手動計數 + QR碼掃描 + 贈品 |
| **產生的潛在客戶** | 500+ 合格聯絡人 | 表格、名片交換、註冊 |
| **媒體印象** | 300,000+ 瀏覽量 | 來自視頻、文章、社交媒體帖子的分析 |
| **媒體提及** | 5+ 文章/8+ 採訪 | 媒體監控 |

## 參與與互動

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **研討會參與者** | 100+ 人 |45分鐘研討會期間計數 |
| **演講觀眾** | 300+ 人 | 15分鐘演講時段期間計數 |
| **分發的商品** | 500+ 件 | 庫存前後對比 |

## 社群成長

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **新Discord成員** | 1,000+ 通過QR碼 | Discord分析 + 贈品 |
| **新大使** | 10+ 招募 + 200~500 社群成員 | 大使資料庫 + 贈品 |
| **社交媒體關注者** | +1,000 Qubic +1.000 Qubic France | 平台分析 |
| **電子報** | 100k (cointribune + CryptoXR) | 電子郵件分析 |

## ROI與轉化

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **分發的QUBIC** | 2億QUBIC贈送 | 分發追蹤 |
| **建立的合作夥伴關係** | 10+ 新合作夥伴 | 商業跟進 + KOL |
| **開發者興趣** | 3+ | 社群SC + 生態系統RFP |

## 側邊活動（共同主辦）

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **側邊活動參與者** | 800+ 人 | 入場計數 |
| **合格網絡建設** | 100+ 預定會議 | 參與者議程 |
| **動員的合作夥伴** | 學校 + 挖礦 + 機構 + 媒體（Cointribune） | 參與確認 + 文章 |

## 活動後指標

| KPI | 目標 | 測量方法 |
|-----|--------|--------------------|
| **活動視頻** | 100,000+ 瀏覽量/視頻 | YouTube/社交媒體 |
| **用戶生成內容** | 50+ 社群帖子 | 標籤監控 |
| **商業管道** | $100K+ 機會 | 追蹤/聯絡 |
| **展台可重複使用性** | 2026年3+ EMEA活動 | 活動規劃時間表 |

## 戰略目標

| 目標 | 指標 | 目標值 |
|-----------|--------|--------|
| **法國市場領導地位** | 對比競爭對手的聲音份額 | 前3名提及 |
| **開發者生態系統** | 參與的Epitech/機構開發者 | 50+ 開發者（學校）+ 機構 |
| **技術採用** | 演示 | 覆蓋200+ 人 |
| **歐洲存在** | EMEA媒體報導 | 覆蓋5+ 國家 |
| **社群成熟度** | 網絡建設 | 2026年3個新的大型低成本活動 + 通過大使的小型活動 |
| **國家熱潮** | KOL + 媒體 | 10+ 新合作夥伴關係（20個視頻）+ 2個新媒體免費聯絡（2篇文章） |


## 預算對比績效

**分配預算**：143億QUBIC（$22,500美元）

**預期ROI**：
  - 每印象成本：< $0.05
    
  - 每合格潛在客戶成本：< $4.5
    
  - 投資回報率：300%+ 產生價值

**財務追蹤**：所有剩餘資金將退還給CCF


這些目標與QUBIC的生態系統成長、利益相關者參與和技術採用目標一致。

*所有資金用於外部付費曝光。無團隊薪水或管理費用。*



















